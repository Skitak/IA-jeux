...
#include "../Buckland_Chapter7 to 10_Raven/resource.h"
                         ...
                         
{
  ...
  //setup the network
  m_pNet = new CNeuralNet(NUM_VECTORS,        //inputs
...
void CController::AddData(vector<double> vector, double result)
{
	m_pData->AddData(vector, result);
}

void CController::NewNetwork()
{
	...
	m_pNet = new CNeuralNet(NUM_VECTORS,
		m_iNumValidPatterns,
		NUM_VECTORS,
		LEARNING_RATE);
}
      ...
      else
      {
        ...
        if(MessageBox(m_hwnd, "Happy with this gesture?", "OK?", MB_YESNO) == IDYES)
        {
          ...
          DialogBox(hInstance,
           ...
           //add the data
           m_pData->AddData(m_vecVectors, 0);//tire pas pour l'instant
...
bool CController::TestForShoot(vector<double> vecTest)
{
	//input the smoothed mouse vectors into the net and see if we get a match
	vector<double> outputs = m_pNet->Update(vecTest);
	...
	string s = m_pData->PatternName(m_iBestMatch);
	if (s == "shoot") {
		return true;
	}
	return false;
...
void CController::Render(HDC &surface, int cxClient, int cyClient)
{
  ...
  //      {
		...
		//	s = m_pData->PatternName(m_iBestMatch);
  //      }

  //      else
  //      {
		//	SetTextColor(surface, RGB(0, 0, 255));
		//	s = m_pData->PatternName(m_iMatch);
  //      }
		//
		//if (s == "1") {
		//	TextOut(surface, 5, 10, "shoot", 5);
		//}
		//else if (s=="0"){
		//	TextOut(surface, 5, 10, "don't shoot", 11);
		//}
		//else {
  ...
  {
    for (int vtx=0; vtx<m_vecSmoothPath.size(); ++vtx)
    {
